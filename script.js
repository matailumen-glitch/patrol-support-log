    ...
    secondArea.innerHTML=
      `<div class="managementTitle">${escapeHtml(courseTitle)}（${currentSessionNo}回目巡回済）</div>
       ...
       <div class="managementInfoGrid">
         <div class="infoCard">
           <div class="infoLabel">📍 未巡回の工場数</div>
           <div class="infoValue dangerValue">${remain.length} 工場</div>
         </div>
         <div class="infoCard">
       ...
       <button class="managementButton green" onclick="prepareSecondSession()">
         ...
         <span class="subText">未巡回の工場から巡回を再開します</span>
       </button>
       ...
       <div class="guideSmall">ℹ️「${nextNo}回目巡回を開始」を押すと、未巡回の工場のみで巡回を再開できます。</div>
...
function makeSessionHistoryBlock(session){
  ...
  summary.innerHTML=
    `<div><span>出発</span><strong>${formatTime(session.departTime)}</strong></div>
     ...
     <div><span>未巡回</span><strong>${session.remainingNames.length} 件</strong></div>`;
